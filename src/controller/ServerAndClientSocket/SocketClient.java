...
//	private OutputStream outputStream;
	...
	private Consumer<ChatMessage> setImageMessageHandler;
...
//	}
	...
	// Ha - SendImages
	public void setImageMessageHandler(Consumer<ChatMessage> handler) {
		this.setImageMessageHandler = handler;
	}
		...
		new Thread(() -> {
			try {
				...
				while ((msg = in.readUTF()) != null) {
					...
					}else if(packet.getType().equals("IMAGE_UPLOAD_RESULT")) {
						ChatMessage chatMessage = gson.fromJson(gson.toJson(packet.getData()), ChatMessage.class);

						if (fileMessageHandler != null) {
							setImageMessageHandler.accept(chatMessage);
						}
					}
...
// Ha - sendImage
	public void sendImage(File file, String senderId, String receiverId) {
		new Thread(() -> {
			try {
				FileInputStream fis = new FileInputStream(file);
				...
				Packet packetRequest = new Packet();
				packetRequest.setType("IMAGE_META");
				packetRequest.setData(chatMessage);
