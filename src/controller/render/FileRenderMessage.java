...
public class FileRenderMessage {
	...
	public void setImagesLocal(File file) {
	    ...
	    if (lowerName.endsWith(".jpg") || lowerName.endsWith(".jpeg") || lowerName.endsWith(".png")) {
	        ...
	        // Thêm sự kiện click
	        ImagesView.setOnMouseClicked(e -> {
	            try {
	                if (Desktop.isDesktopSupported()) {
	                    Desktop.getDesktop().open(file); // mở bằng app mặc định
	                } else {
...
// Ha - SetImagesInfo
	...
	public void setImageInfo(FileInfo info) {
	    ...
	    if (lowerName.endsWith(".jpg") || lowerName.endsWith(".jpeg") || lowerName.endsWith(".png")) {
	        // load ảnh từ Cloudinary (hoặc local path)
	        ImagesView.setImage(new Image(info.getUrlUpload(), true));
	        ImagesView.setVisible(true);
	        ...
	        ImagesView.setOnMouseClicked(e -> {
	            try {
	                if (Desktop.isDesktopSupported()) {
	                    // Mở trực tiếp link URL
	                    Desktop.getDesktop().browse(new java.net.URI(info.getUrlUpload()));
	                } else {
	                    System.err.println("Desktop API không được hỗ trợ trên hệ điều hành này.");
	                }
	            } catch (Exception ex) {
