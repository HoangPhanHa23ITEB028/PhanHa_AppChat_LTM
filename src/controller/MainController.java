...
//	    Platform.runLater(() -> vboxInScroll.getChildren().add(fileBubble));
	...
	//Ha - sendImages
	//Ha - sendImages
	public void renderImagesMessage(ChatMessage chatMessage, boolean isSend) throws IOException {
	    FileInfo fileData = socketClient.gson.fromJson(chatMessage.getContent(), FileInfo.class);

	    FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/component/ShowImages.fxml"));
	    Node fileBubble = loader.load();

	    Map<Boolean, styleDifferenceClass> mapStyleMessenger = new HashMap<>();
	    mapStyleMessenger.put(true, new styleDifferenceClass(Pos.TOP_RIGHT,
	            "-fx-background-color: rgb(15,125,242); -fx-background-radius: 20px;"));
	    mapStyleMessenger.put(false, new styleDifferenceClass(Pos.TOP_LEFT,
	            "-fx-background-color: rgb(233,233,235); -fx-background-radius: 20px;"));

	    FileRenderMessage controller = loader.getController();
	    controller.setImageInfo(fileData); // truyền dữ liệu vào controller
	    ...
	    if (!isSend) {
	        ...
	        // bọc avatar trong VBox để đẩy lên top
	        VBox avatarBox = new VBox(avatar);
	        avatarBox.setAlignment(Pos.TOP_CENTER);

	        hBox.getChildren().addAll(avatarBox, fileBubble);
	    } else {
	        hBox.getChildren().add(fileBubble);
	    }
...
//	    vboxInScroll.getChildren().add(messageBox);
	...
	// Ha - sendImages
	@FXML
	void openImages(MouseEvent event) throws IOException {
		FileChooser fileChooser = new FileChooser();
		fileChooser.setTitle("Chọn file để gửi");
...
//				new FileChooser.ExtensionFilter("Tài liệu", "*.pdf", "*.docx", "*.txt"));
		...
		if (selectedFile != null) {
			...
			XemHinhClient(selectedFile);

			socketClient.sendImage(selectedFile, user.getIdHex(), "all");
	...
	// Ha - sendImages
	public void XemHinhClient(File file) throws IOException {
	    FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/component/ShowImages.fxml"));
	    Node ImagesData = loader.load();

	    FileRenderMessage controller = loader.getController();
	    controller.setImagesLocal(file); // truyền dữ liệu vào controller

	    HBox hBox = new HBox(ImagesData);
	    hBox.setAlignment(Pos.CENTER_RIGHT); // Canh phải
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		socketClient.setImageMessageHandler(chatMessage -> {
			try {
				renderImagesMessage(chatMessage, false);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
